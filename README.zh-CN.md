...
| `scene3d/View3D` | **三维视图**：软件渲染，无 GPU 依赖。左键旋转 / 滚轮缩放 / Shift+左键平移；单击拾取部件（拾取复用当帧已排序的面表，**点到的一定是看见的那个**）|
| `scene3d/Scene3D` | 场景与部件表（**非控件，不进控件树**）。`setPartState(id, 状态)` 给部件着色；状态只存语义，颜色绘制时按主题解析 |
