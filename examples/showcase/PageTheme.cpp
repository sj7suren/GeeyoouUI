...
// A FUNCTION, not a namespace-scope const.
//
// A const here is initialised once, in whatever language the process
// started in -- and the theme page would then hand an operator running
// the English UI a Chinese sample sheet.  Rebuilding is cheap: it runs
// once per visit to this page.
std::string sampleQss() {
  return tr("/* 选择器示例：改完点「应用样式表」 */\n"
    "\n"
    "/* 按 objectName 选中单个控件 */\n"
    "#emergencyStop {\n"
    "  accent: #FF3B30;\n"
...
Size buildThemePage(Widget* content, ShowcaseWindow& win) {
  ...
  cSkin->currentValueChanged.connect([say](const std::string& name) {
    skins().apply(name);
    say(tr("已切换皮肤：") + name + tr("（整棵控件树立即重绘）"));
  });
  ...
  gAccent->setTitle(tr("主题色"));

  const std::vector<NamedColor> kAccents = {
      {tr("仪表蓝"), Color::rgb(0x2F, 0xA8, 0xFF)},
      {tr("品牌青"), Color::rgb(0x12, 0xC2, 0xC2)},
      {tr("安全绿"), Color::rgb(0x3E, 0xD1, 0x7A)},
      {tr("工程橙"), Color::rgb(0xFF, 0x8A, 0x1E)},
      {tr("警示红"), Color::rgb(0xFF, 0x4D, 0x5E)},
      {tr("品牌紫"), Color::rgb(0x8B, 0x5C, 0xF6)},
  };
  caption(gAccent, 14, 42, 272, tr("一个颜色带动整套配色"));
  ...
  cAccent->currentIndexChanged.connect([say, kAccents](int i) {
    if (i < 0 || i >= int(kAccents.size())) return;
    skins().setAccent(kAccents[std::size_t(i)].color);
    say(std::string(tr("主题色 = ")) + kAccents[std::size_t(i)].name +
        tr("（accent / primary / focusRing / 选区 一起走）"));
  });
  ...
  accentNote->setText(
      tr("只有派生自品牌色的 token 会动：accent / primary / "
  ...
  apply->clicked.connect([editor, say, &win] {
    ...
    if (errs.empty()) {
      say(tr("样式表已应用：") + std::to_string(activeStyleSheet().ruleCount()) +
          tr(" 条规则生效"));
    } else {
      ...
      say(tr("样式表有 ") + std::to_string(errs.size()) + tr(" 处问题：") + errs.front());
    }
  });
  reset->clicked.connect([editor, say, &win] {
    editor->setText("");
    win.setUserStyleSheet("");
    say(tr("样式表已清空，只剩皮肤自带规则与 Theme token"));
  });

  // ------------------------------------------------------------ 受控的样本 ---
  auto* gDemo = content->add<GroupBox>();
  gDemo->setGeometry({0, 270, 620, 320});
  gDemo->setTitle(tr("样本控件（上面的规则作用在这里）"));
  ...
  stop->setText(tr("紧急停车"));
  stop->setVariant(ButtonVariant::Danger);
  stop->setObjectName("emergencyStop");

  caption(gDemo, 180, 42, 356, tr(".pill —— 按 style class 命中"));
  ...
  plain->setText(tr("未加类"));

  caption(gDemo, 14, 116, 280, tr("#styleDemo ProgressBar —— 后代选择器"));
  auto* bar = gDemo->add<ProgressBar>();
  bar->setGeometry({14, 138, 400, 18});
  bar->setValue(64);

  caption(gDemo, 14, 168, 280, tr("跟随 accent 的控件"));
