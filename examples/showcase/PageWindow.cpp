...
Size buildWindowPage(Widget* content, ShowcaseWindow& win) {
  ...
  log->setText("提示：按住标题栏空白处即可拖动窗口，双击可最大化 / 还原。");
  ...
  gMetrics->setTitle("标题栏度量");

  caption(gMetrics, 14, 42, 272, "标题栏高度");
  ...
  sHeight->valueChanged.connect([h, say](double v) {
    h->setHeight(float(v));
    say("标题栏高度 = " + std::to_string(int(v)) + " px");
  });

  caption(gMetrics, 14, 100, 272, "窗口按钮宽度");
  ...
  sBtn->valueChanged.connect([h, say](double v) {
    ...
    say("窗口按钮宽度 = " + std::to_string(int(v)) + " px");
  });

  caption(gMetrics, 14, 158, 272, "左侧留白");
  ...
  gBrand->setTitle("标题与图标");

  caption(gBrand, 14, 42, 272, "标题文本");
  ...
  static const std::vector<std::pair<const char*, Icon>> kIcons = {
      {"设置齿轮", Icon::Settings},        {"信息", Icon::Info},
      {"报警", Icon::Warning},             {"运行", Icon::Play},
      {"锁定", Icon::Lock},                {"全球", Icon::Globe},
      {"泵（自定义）", icons().find("pump")},
      {"阀（自定义）", icons().find("valve")},
      {"罐（自定义）", icons().find("tank")},
      {"闪电（SVG）", icons().find("zap")},
      {"无图标", Icon::None},
  };
  ...
  cIcon->currentIndexChanged.connect([h](int i) {
  ...
  static const std::vector<Swatch> kBg = {
      {"面板灰（默认）", Theme::current().panel.lerp(Theme::current().background, 0.2f)},
      {"深空蓝", Color::rgb(0x0E, 0x1B, 0x2E)},
      {"石墨黑", Color::rgb(0x16, 0x16, 0x1A)},
      {"工业绿", Color::rgb(0x0F, 0x24, 0x1C)},
      {"警示棕", Color::rgb(0x2A, 0x1C, 0x10)},
  };
  caption(gColor, 14, 42, 272, "标题栏底色");
  ...
  cBg->currentIndexChanged.connect([h](int i) {
    if (i >= 0 && i < int(kBg.size())) h->setBackground(kBg[std::size_t(i)].color);
  });

  static const std::vector<Swatch> kAccent = {
      {"仪表蓝（默认）", Theme::current().accent},
      {"安全绿", Theme::current().ok},
      {"警戒橙", Theme::current().warn},
      {"报警红", Theme::current().alarm},
  };
  caption(gColor, 14, 102, 272, "图标 / 强调色");
  ...
  cAccent->currentIndexChanged.connect([h, &win](int i) {
  ...
  gIcons->setTitle("图标扩展（IconRegistry）—— 上面「图标」下拉里的自定义项就是这些");
  ...
  iconNote->setPixelSize(11.0f);
  {
    ...
    iconNote->setText(
        errs.empty()
            ? ("已注册自定义图标 " + std::to_string(icons().customCount()) +
               " 个，SVG 路径全部解析通过；名字可直接 icons().find(\"pump\") 取回")
            : ("SVG 解析有 " + std::to_string(errs.size()) + " 处问题：" + errs.front()));
  }
